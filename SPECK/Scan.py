...
class Scan:
    ...
    def extract(self, file):
        ...
        try:
            ...
            subprocess.call(
                "ps -a | grep \"apk2src\" | awk '{print $1}' | xargs kill 2> /dev/null;",
                shell=True,
            )

        self.directory = file.replace(".apk", "")
        if not (
            os.path.isfile(self.directory + "/resources/AndroidManifest.xml")
        ):
            print(
                f"\033[31m[-] {file.split('/')[-1]} extraction failed\033[0m"
            )
    ...
    def analyse(self):
        ...
        for rule in self.rules:
            ...
            StoreJson.store(
                "rulestats",
                {"apk": apk, "rule": rule, "start": ruleStart, "end": ruleEnd},
                self.database,
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s", "--source", help="App directory / Apk file", required=True
    )
    parser.add_argument("-r", "--rules", nargs="+", help="rules numbers")
    parser.add_argument(
        "-t", "--timeout", help="Time before killing bash process", type=int
    )
    parser.add_argument(
        "-g", "--good", help="Display good practices", action="store_true"
    )
    parser.add_argument(
        "-f",
        "--flowdroid",
        help="Use FlowDroid to detect false positive / negative",
        action="store_true",
    )
    parser.add_argument("-p", "--platform", help="Android SDK platform folder")
    parser.add_argument(
        "-q", "--quiet", help="Output to parse", action="store_true"
    )
    parser.add_argument(
        "-D", "--database", help="database where to save all violations"
    )
