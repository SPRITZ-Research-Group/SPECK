...
class Rule19(Rules):
    def __init__(
        ...
        self.AndroidErrMsg = (
            "service(s) which can be invoked by other apps (don't) explicit it"
        )
        self.AndroidOkMsg = (
            "service(s) which can be invoked by other apps (are) well declared"
        )
        ...
        self.errMsg = (
            "Service has to explicitly use android:exported attribute"
        )
                ...
                if serviceName.split(".")[-1] in clss[R.INSTR]:
                    ...
                    for func in listFunc:
                        if func[R.CLASSID] == clss[R.CLASSID]:
                            if any(
                                elem in func[R.INSTR] for elem in whitelist
                            ):
                                isOk = True
                                break
