...
class Rule25(Rules):
    def __init__(
        ...
        self.AndroidErrMsg = (
            "SSLSocket (don't) perform() hostname verification"
        )
        ...
        for elem in sslSession:
            if s[R.VALUE] == elem[R.VALUE]:
                if elem[R.SCOPE] == R.VARGLOBAL:
                    ...
                    and elem[R.CLASSID] == s[R.CLASSID]
                ):
                    return elem
                elif (
                    elem[R.SCOPE] == R.VARLOCAL
                    and elem[R.CLASSID] == s[R.CLASSID]
                    and elem[R.FUNCID] == s[R.FUNCID]
                ):
                    return elem
        return None
    ...
    def checkIfVerifyIsCalled(self, fileReader, host, sslSession):
        ...
        for h in host:
            sslSessionArg = Parser.finder(
                fileReader,
                [[Parser.findArgName, (h[R.VALUE] + ".verify", 1, None)]],
                True,
            )[0]
    ...
    def run(self):
        ...
        for f in self.javaFiles:
            ...
            found = Parser.finder(
                ...
                [
                    ...
                    [
                        Parser.findVarName,
                        (["(SSLSocket)"], None),
                    ],  # needed if socket is create but not assigned to a var. May cause the issue to be reported twice, but at least it is not ignored
            ...
            host, NotIn2 = Parser.diff(
                hostnameVerifier, defaultHostnameVerifier
            )

            In, NotIn3 = self.checkIfVerifyIsCalled(
                fileReader, host, sslSession
            )
            ...
            self.updateOCN(
                ...
                In,
                NotIn + NotIn2,
                (len(In) == 0 and len(NotIn + NotIn2) == 0),
            )
            self.loading()
            fileReader.close()
