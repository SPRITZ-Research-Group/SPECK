...
def flowdroid_error():
    ...
    if not flowdroid_err[0]:
        ...
        flowdroid_enabled[0] = False

class Rules():
    def __init__(self, directory, database, verbose=True, verboseDeveloper=False, storeManager=None, flowdroid=False, platform="" , validation=False, quiet=True, flowdroid_cb=True):
    ...
    def filter(self, package, notFilt=False):
        if notFilt:
            ...
            self.javaFiles = filt.execute() # "filter" all the files, getting only the ones importing the "interesting package"
            self.maxFiles = len(self.javaFiles)
    ...
    def findXml(self, incr=True):
        ...
        for f in xmlFiles:
            if "/build/intermediates/merged_manifest/debug/AndroidManifest.xml" in f or "/resources/AndroidManifest.xml" in f:
                if incr:
                    self.maxFiles = 1
                self.manifest = f
                return
    ...
    def store(self, Id=-1, AndroidOkMsg="", AndroidErrMsg="", AndroidText="", category=R.CAT_NA, withBol=False, err=[]):
        # FLOWDROID
        if self.flowdroid and flowdroid_enabled[0] and self.platform != "":
            ...
            if not os.path.isfile(apk):
                ...
                if len(apks) > 0:
                    apk = apks[0]
            fdroid = FlowDroid(Id, apk, self.platform, self.verbose, self.verboseDeveloper, err, cb=not (self.flowdroid_cb and flowdroid_err[0]))
    ...
    def display(self, Reader=None):
        if self.verbose:
            if self.quiet:
                ...
                if self.directory.endswith("/"):
                    apk = self.directory[:-1]
                StoreJson.showResults(self.results, self.statFiles, Reader, self.packageDir, self.validation, self.nb, apk, mongo_cfg=self.database)
            else:
    ...
    def just_update(self, filename, NotIn):
        ...
        try:
            severity = NotIn[0]['eType']        # because NotIn is a list of dicts (very often of one dict)
        ...
        data = new_data()
        data[severity] = NotIn
        self.results.append([filename, data])   # maybe I have to insert this part also in the prev funct (e.g. updateC, updateON, ...)
