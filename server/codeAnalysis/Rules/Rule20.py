...
? Pseudo Code:
	1. Check for <intent-filter> inside <service>
...
class Rule20(Rules):
	...
	def run(self):
		...
		if self.manifest != None:
			...
			services = xmlReader.getArgsCond("service", "intent-filter")
			In, NotIn = self.checkExported(services)
			...
			self.updateOWN(xmlReader.getFile(), In, In, (len(In) == 0 and len(In) == 0))
